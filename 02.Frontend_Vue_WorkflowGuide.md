...
<template>
    <div>
        ...
        <!-- $route.query：URL 查询参数对象（? 后面的键值对）。
             ...
             params 与 query 的区别：
               params → 路径的一部分（/master/100），刷新页面仍存在（需配合 name 跳转）。
               query  → URL 查询字符串（?keyword=vue），刷新页面仍存在，适合搜索、筛选参数。 -->
...
methods: {
    goToPage() {
        ...
        // 注意：使用 path 时 params 会被忽略，传参必须用 name + params 或 path + query。
        this.$router.push({ name: 'master', params: { count: 50 } });
