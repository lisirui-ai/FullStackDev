...
本向导以 **课程配套资料（Vue 2 Options API）** 为主线，同时标注 Vue 3 差异，
...
> **版本说明（重要）**：
> - **Vue 2**（课程主线）：最终版为 **2.7.16**，已于 **2023-12-31 正式 EOL**，不再维护。
>   课程内容基于 Vue 2 Options API，仍可学习和运行，但新项目请迁移至 Vue 3。
> - **Vue 3**（生产推荐）：当前稳定版为 **3.5.40**（2026-07-16），3.6.0 RC 阶段。
>   官方文档：[https://cn.vuejs.org/](https://cn.vuejs.org/)
> - **Vue CLI**（课程使用）：已进入 **Maintenance Mode（仅维护不新增功能）**。
...
# ============================================================
# 第三步（课程方式）：安装 Vue CLI（已进入 Maintenance Mode）
# ⚠️  Vue CLI 已停止新功能开发，仅维护；用于运行本课程 Vue 2 项目
# ============================================================
npm install -g @vue/cli           # 安装 Vue CLI（@vue/cli 4.x/5.x）

# 验证 Vue CLI 安装
vue --version                     # 打印 @vue/cli 版本，如 5.x.x
...
```bash
# ============================================================
# 方式一（课程 / Vue 2 项目）：使用旧版 vue-cli（vue init webpack）
# ⚠️  vue-cli（不是 @vue/cli）已废弃，仅用于复现课程环境
...
<!DOCTYPE html>
<!-- ============================================================
   DOCTYPE 声明：告诉浏览器此文档使用 HTML5 标准解析
   html lang="zh-CN"：声明页面语言为简体中文（SEO 友好）
   ============================================================ -->
<html lang="zh-CN">
<head>
    <!-- meta charset：声明字符编码为 UTF-8，防止中文乱码 -->
    <meta charset="UTF-8">
    <!-- viewport：移动端适配，禁止用户缩放，初始缩放比为 1 -->
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <!-- title：显示在浏览器标签页上的网页标题 -->
    <title>网页标题</title>
    <!-- link：外联 CSS 样式文件 -->
    <link rel="stylesheet" type="text/css" href="css/main.css">
    <!-- script：引入外部 JS 文件（也可放在 body 底部）-->
...
<dl>                            <!-- 描述列表（Description List） -->
    <dt>HTML</dt>               <!-- 描述术语（Description Term） -->
    <dd>负责页面结构</dd>        <!-- 术语的解释（Description Description） -->
</dl>
...
<form action="http://www.example.com/submit" method="post">
    ...
    <p>
        ...
        <!-- textarea：多行文本输入框；rows 行数，cols 列数 -->
    </p>
...
* { margin: 0; padding: 0; }        /* 通配符：选中所有元素，常用于 reset */
...
/* --- 6. 伪类与伪元素选择器 ---
   伪类（:）：表示元素的特殊状态（如鼠标悬停）
   伪元素（:before/:after）：在元素前/后插入内容 */
a:hover { color: gold; text-decoration: underline; } /* 鼠标悬浮时的样式 */
.box1:before { content: "前缀文字"; color: blue; }   /* 在元素前插入内容 */
.box2:after  { content: "后缀文字"; color: blue; }   /* 在元素后插入内容 */
```
...
┌─────────── margin（外边距，透明，不可设颜色） ───────────┐
│  ┌───────── border（边框，可设颜色和样式） ─────────┐    │
│  │  ┌────── padding（内边距，与 content 同色） ───┐ │    │
│  │  │  ┌── content（内容区，width/height 决定） ┐ │ │    │
│  │  │  │          内容文字或图片                │ │ │    │
│  │  │  └────────────────────────────────────────┘ │ │    │
│  │  └─────────────────────────────────────────────┘ │    │
│  └────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────┘
盒子真实宽度 = width + 左右 padding + 左右 border
...
.box {
    ...
    border-collapse: collapse;          /* 表格边框合并（两条边变一条） */
...
> **版本对照（2026）**：
> - **Vue 2.7.16**：最终版，2023-12-31 EOL，课程主线，Options API，不再维护
