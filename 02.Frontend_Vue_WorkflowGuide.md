...
>
> **注意**：`class` 是 JS 保留字，对应的 DOM 属性名改为 `className`；自定义特性（如 `data-*`）在 DOM 对象上无对应属性，必须通过 `getAttribute()` 读取。
...
window.clearTimeout(nTimer);        // 取消尚未触发的 setTimeout

// setInterval：每隔 ms 毫秒重复执行回调，返回定时器 ID（number）
var nInterval = window.setInterval(function() {
    console.log('每秒执行一次');
}, 1000);
window.clearInterval(nInterval);    // 停止 setInterval 的重复执行
...
> **`<template>` 配合 `v-for`**：`<template>` 是 Vue 的虚拟容器，不会渲染成真实 DOM 节点，在其上使用 `v-for` 可以循环渲染一组元素而不引入多余的包裹标签。注意 Vue 2 中 `:key` 不能写在 `<template>` 上（它不产生真实节点，无处挂 key），需写在内部每个子元素上；Vue 3 已支持直接写在 `<template>` 上。
...
<template>
    <div>
        ...
        <!-- checkbox 单个复选框：v-model 绑定布尔值，选中时 checked=true，取消时 checked=false -->
        <input type="checkbox" v-model="checked">
        <!-- radio 单选框：同一组 radio 的 v-model 绑定同一个变量 gender -->
        <!-- 选中时将该 input 的 value 属性值（"male" 或 "female"）赋给 gender -->
...
export default {
    directives: {
        ...
        mycss: {
            inserted(el) { el.style.color = 'red'; } // el 挂载到 DOM 后将字体颜色设为红色
        }
    }
}
...
export default new Router({
    mode: 'history', // 使用 HTML5 history 模式（URL 不含 #），生产环境需 Nginx 配置 try_files
    routes: [
        { path: '/',           name: 'home',   component: HomePage   }, // 首页路由
        { path: '/detail/:id', name: 'detail', component: DetailPage } // 详情路由，:id 为动态参数
    ]
})
```
...
export default new Vuex.Store({
    ...
    mutations: {
        setNews(state, list)    { state.newsList = list; },   // 将请求回来的新闻数组写入 state
        setLoading(state, flag) { state.loading = flag; }     // 切换加载状态：true=加载中，false=完成
    },
    actions: {
        fetchNews(context) {
            context.commit('setLoading', true);  // 请求开始前先将 loading 设为 true，页面显示"加载中"
            return Axios.get('/api/news/').then(res => {
                context.commit('setNews', res.data);     // 将响应数据写入 state.newsList
                context.commit('setLoading', false);     // 请求完成后关闭加载状态
            });
        }
    },
    getters: {
        // 对 state.newsList 派生计算，返回列表长度；组件通过 this.$store.getters.newsCount 访问
