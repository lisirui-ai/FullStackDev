...
> **`v-bind`（`:`）的引号内写的是 JavaScript 表达式**，因此 data 变量和任意合法的 JS 语法（运算、拼接、三元、方法调用等）均可直接使用：
>
> ```vue
> <p :style="{ fontSize: textSize + 'px' }">  <!-- textSize 是 data 变量，+ 'px' 是 JS 拼接 -->
> <div :class="isActive ? 'active' : 'inactive'">  <!-- 三元表达式 -->
> ```
...
<template>
    <div>
        ...
        <!-- 对象语法（:class / :style 专属增强，不适用于其他属性）：{ class名: 布尔值 }，布尔值为 true 时添加该 class，false 时移除 -->
        <!-- isActive=true、isDisabled=false → 渲染为 class="active" -->
        <div :class="{ active: isActive, disabled: isDisabled }">对象语法</div>
        <!-- 数组语法（:class / :style 专属增强，不适用于其他属性）：[ 字符串变量, { class名: 布尔值 } ]，字符串变量直接追加，对象按布尔值决定是否追加 -->
        <!-- baseClass='base'、isHighlight=true → 渲染为 class="base highlight" -->
